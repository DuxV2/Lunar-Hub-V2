...
do
    ...
    function z.onPhysics(Q, R, S)
        ...
        if T > 0 and Toggles["character.sprint.enabled"].Value and Options["character.sprint.keybind"]:GetState() then
...
do
    ...
    function B.onStart(ab)
        ...
        aa.onStart(ab)
        ab._maid:add(
            ab.instance.CharacterAdded:Connect(
                function(ac)
                    ab:onCharacter(ac)
                end
            )
        ...
        ab._maid:add(
            ab.instance.Backpack.ChildAdded:Connect(
                function(ac)
                    if ac:IsA "Tool" then
                        ab:onTool(ac)
                    end
    ...
    function C.onRender(O, P)
        ...
        if Q then
            ...
            if U.Z < 0 then
                ...
                V = ad(0, 0, W):VectorToWorldSpace((ad(0, math.rad(89.9), 0):VectorToWorldSpace(ac(0, 0, -1))))
...
do
    ...
    function D.onStart(T)
        ...
        U.Visible = false
        U.Thickness = 1
        U.Color = T.name == "Rush" and Options["esp.rush.rush-color"].Value or Options["esp.rush.ambush-color"].Value
        task.delay(
            0.5,
            function()
                ...
                if V <= 750 then
                    N(
                        Toggles["alerts.entity"].Value,
                        {
                            message = T.name == "Rush" and "Rush spawned, hide in a Wardrobe or get behind cover!" or
...
do
    ...
    function J.onStart(ab)
        ...
        ab._maid:add(
            ab.instance.DescendantAdded:Connect(
                function(ad)
                    ab:onDescendant(ad)
                end
            )
...
m:SetWatermark "Linoria Community (OminousVibes)"
m:Notify "Loading UI..."
do
        ...
        do
            ...
            ad:AddToggle(
                "alerts.enabled",
                {
                    Text = "Alerts Enabled",
                    Default = true,
                    Tooltip = "Master toggle for all script alerts."
                }
            )
            ad:AddToggle(
                "alerts.audio",
            ...
            ae:AddToggle(
                "alerts.debug",
                {
                    Text = "Debug Alerts",
                    Default = true,
                    Tooltip = "Enabled alerts related to script status."
                }
            )
            ae:AddToggle(
                "alerts.entity",
                {
                    Text = "Entity Alerts",
                    Default = true,
                    Tooltip = "Enabled alerts about entities spawning."
                }
            )
        end
    end
    do
        ...
        do
            ...
            do
                ...
                ):AddColorPicker("esp.player.color", {Title = "Player ESP Color", Default = Color3.new(1, 1, 1)})
                ad:AddToggle(
                    "esp.screech.enabled",
                    {
                        Text = "Screech ESP",
                        Default = true,
                        Tooltip = "Tracer ESP for Screech entities"
                    }
                ):AddColorPicker(
                    "esp.screech.color",
                    {
                ...
                ):AddColorPicker("esp.rush.ambush-color", {Title = "Ambush ESP Color", Default = Color3.new(0, 1, 0)})
                ad:AddToggle(
                    "esp.figure.enabled",
                    {
                        Text = "Figure ESP",
                        Default = true,
                        Tooltip = "ESP for the Figure entity"
                    }
                ):AddColorPicker(
                    "esp.figure.color",
                    {
            ...
            do
                ...
                ):AddColorPicker("esp.wardrobe.color", {Title = "Wardrobe ESP Color", Default = Color3.new(0, 1, 0.9)})
                ae:AddToggle(
    ...
    do
        ...
        ac:AddLabel "OminousVibes - Creator"
    ...
    do
        ...
        ac:AddLabel "Menu bind":AddKeyPicker(
            "MenuKeybind",
            {
                Default = "End",
                NoUI = true,
...
d.Stepped:Connect(T)
Options["gameplay.parser.keybind"]:OnClick(
    function()
        local U = ""
        for V = 1, 5 do
            local W = w[x[V]]
        ...
        N(
            Toggles["gameplay.parser.enabled"].Value,
            {
...
do
    ...
    if V > W then
        ...
        setclipboard("[" .. tostring(U) .. "] = " .. tostring(V) .. ",")
    end
end
